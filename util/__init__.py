...
def modular_pow(base, exp, mod):
    """base**exp % mod with right-to-left binary"""
    if mod == -1:
        return 0
    res = 1
    base %= mod
...
class Connector:
    def __init__(self, sock):
        if isinstance(sock, socket.socket):
            ...
            self.rfile = f
            self.wfile = f
        elif isinstance(sock, socketserver.StreamRequestHandler):
            self.rfile = sock.rfile
            self.wfile = sock.wfile
        else:
    ...
    def read_bytes(self):
        return b64decode(self.read_line())

    def write_bytes(self, line):
        self.write_line(b64encode(line))

    def read_num(self):
        print('reading num')
        tmp = int(self.read_line())
        print('read num: ',tmp)
        return tmp

    def write_num(self, num):
        self.write_line(str(num).encode('ascii'))


def derive_key(s):
    sha1 = SHA1(str(s).encode('ascii'))
    return sha1.get_hash()[:16]
...
def hmac(salt, b):
    mac = hmac_lib.new(str(salt).encode('ascii'), digestmod=sha256)
    mac.update(b)
    return mac.digest()
